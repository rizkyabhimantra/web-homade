...
class AchievementExport implements FromQuery, ShouldAutoSize, ShouldQueue, WithCustomStartCell, WithDefaultStyles, WithHeadings, WithMapping, WithStyles
{
    use Exportable;

    private string $title = 'List Prestasi Homade';

    private int $total_data = 0;

    public function __construct() {
        $this->total_data = (new AchievementService())->all(is_has_limit:false)->count();
        $this->title = $this->title . ' ('. $this->total_data .')';
    }

    public function query()
    {
        return (new AchievementService())->all(
            is_has_limit: false,
            is_query: true,
        );
    }
